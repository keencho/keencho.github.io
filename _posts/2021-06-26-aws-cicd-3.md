...
라우팅 편집화면으로 이동하였다면 0.0.0.0/0 라우팅을 추가하고 대상을 방금 만든 NAT Gateway와 연결해주도록 하겠습니다.
...
만약 외부 업체의 api에 접근해야 하는데 외부 업체에의 보안 정책상 허용할 IP를 알려달라고 하면 방금 만든 NAT Gateway의 EIP를 알려주면 되겠죠?
