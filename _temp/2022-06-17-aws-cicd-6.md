---
title: AWS 와 함께하는 클라우드 기반  CI/CD 서버 구축 - 6. 배포 스크립트 세팅
author: keencho
...
예를들어 CodeDeploy 배포에 실패한 경우 CodeDeploy 에 의해 새롭게 시작된 Auto Scaling Group은 자동종료되지 않기 때문에 이에 대응할 전략이 필요합니다. 제 경우 CodeDeploy의 성공 / 실패 여부를 30초마다 Polling 하는 스크립트를 Jenkins에 포함시켜서 실패한 경우 미리 정의해둔 롤백 람다를 호출하는 방식을 사용하고 있습니다.  

세세한 부분까지 신경쓰지 못했지만 이 포스팅이 AWS를 통해 무중단 배포를 구현하시려는 분들께 도움이 되길 바랍니다.
